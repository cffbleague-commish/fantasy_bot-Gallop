...
// --- Hero profile (player-level: recruiting + awards. Eligibility is per-copy.) ---
  ...
  <div className="pl-hero" style={{ '--accent': accentColor }}>
    ...
    <div className="pl-hero__id">
      <div className="pl-hero__tags">
        <PosChip pos={p.pos} />
        <span className="pl-hero__college">{p.college}</span>
      </div>
      <div className="pl-hero__namerow">
        <h2 className="pl-hero__name">{p.name}</h2>
      </div>
      <div className="pl-hero__meta">{p.ht} {p.wt} · {p.home}</div>
    </div>
...
const App = () => {
  ...
  const [expanded, setExpanded] = useState(() => firstHeld('arch'));
  ...
  useEffect(() => {
    ...
    setExpanded(firstHeld(pid));
  }, [pid]);

  const p = ROSTER.find((r) => r.id === pid);
  const { confs, roll } = LEDGER[pid];
  ...
  return (
    <React.Fragment>
          ...
          <div className={'pl-root pl--' + t.density + ' pl--layout-' + t.layout}>
            ...
            <div className="pl-panel">
              ...
              <div className="pl-ledger">
                ...
                <Layout confs={confs} expanded={expanded} setExpanded={setExpanded}
      ...
      <TweaksPanel>
        ...
        <TweakRadio label="Layout" value={t.layout} options={['rows', 'cards', 'timeline']} onChange={(v) => setTweak('layout', v)} />
        ...
        <TweakRadio label="Prominence" value={t.profile} options={['hero', 'compact']} onChange={(v) => setTweak('profile', v)} />
